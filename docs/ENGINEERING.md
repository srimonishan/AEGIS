...
| Ingestion (container) | Built and ran the real Docker image; hit `/health` and the Meta verification handshake over real HTTP against the running container | `docker build` + `docker run` + `curl` |
...
| Agent orchestrator (container) | Built and ran the real Docker image (all deps incl. `google-adk` installing cleanly); hit `/health` | `docker build` + `docker run` + `curl` |
