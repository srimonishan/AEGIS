...
logger = logging.getLogger("aegis.followup.whatsapp")
...
def send_whatsapp_text(user_id: str, body: str) -> bool:
    ...
    url = f"{_GRAPH_BASE}/{settings.meta_graph_api_version}/{settings.meta_phone_number_id}/messages"
    headers = {
    ...
    try:
        ...
        if resp.status_code >= 300:
            logger.error("whatsapp_send_failed", extra={"status": resp.status_code})
            return False
        return True
